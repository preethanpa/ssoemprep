...
location = '/home/dsie/Developer/sandbox/3ray/3rml/kbc_process/drive_document/'+drive_files_list.get('job_id')+'/pdf/' 
...
def download_drive_file(resource=None, google_file_type='gdoc', target_type=None, target_location=None):
...
if drive_files_list == []:
    ...
    for index, item in enumerate(drive_files_list.files):
        google_file_type = doc_types[item.mimeType]
        target_file_type = "pdf"
        download_drive_file(resource=get_resource(domain_wide_delegate=False), google_file_type=google_file_type, target_type=target_file_type, target_location=location)
