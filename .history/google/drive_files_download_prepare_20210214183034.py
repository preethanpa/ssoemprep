...
def download_drive_file(resource=None, document_id='', google_file_type='gdoc', target_type=None, target_location=None):
...
if drive_files_list == []:
    ...
    for index, item in enumerate(drive_files_list.get('files')):
        try:
            ...
            drive_document_id = doc_types[item.get('mimeType')]
            target_file_type = "pdf"
            download_drive_file(resource=get_resource(domain_wide_delegate=False), document_id=drive_document_id, google_file_type=google_file_type, target_type=target_file_type, target_location=location)
