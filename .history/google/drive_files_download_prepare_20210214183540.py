...
if drive_files_list == []:
    ...
    for index, item in enumerate(drive_files_list.get('files')):
        try:
            ...
            drive_document_id = doc_types[item.get('id')]
            target_file_type = "pdf"
            ddownload_drive_file(resource=get_resource(domain_wide_delegate=False), document_id=drive_document_id, google_file_type=google_file_type, target_type=target_file_type, target_location=location)
