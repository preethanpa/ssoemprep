...
email_id = None if (sys.argv is None or sys.argv[1] is None or sys.argv[1] == 'drive') else sys.argv[2]
if email_id is None:
...
def get_resource(domain_wide_delegate=False, user_to_impersonate=None, resource_type=None):
...
def get_email_labels(user):
    """Shows basic usage of the Gmail API.
    Lists the user's Gmail labels.
    """
    service = get_resource(domain_wide_delegate=True, user_to_impersonate=user, resource_type='gmail')
    # Call the Gmail API
...
def get_email_messages(user):
    """Shows basic usage of the Gmail API.
    Lists the user's Gmail labels.
    """
    service = get_resource(domain_wide_delegate=True, user_to_impersonate=user, resource_type='gmail')
    # Call the Gmail API
    results = service.users().labels().list(user_id=user, includeSpamTrash=None, labelIds=None, maxResults=None, pageToken=None, q=None, x__xgafv=None).execute()
    labels = results.get('labels', [])
