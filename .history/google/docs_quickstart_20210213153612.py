...
def main():
    ...
    request = drive_service.files().export(fileId=DOCUMENT_ID, mimeType=MEDIA_MIME_TYPE_PARAMETER_DEFAULT_VALUE)
    document = docs_service.documents().export(documentId=DOCUMENT_ID).execute()
    print(document)
    # fh = io.BytesIO()
