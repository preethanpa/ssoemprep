...
exc_context = 'phone_number' if json.loads(sys.argv[1])['context'] is None else json.loads(sys.argv[1])['context']
doc_context = 'default_test_doc' if json.loads(sys.argv[1])['doc_name'] is None else json.loads(sys.argv[1])['doc_name']

print(f'exc_context {exc_context} and doc_context {doc_context}')
        ...
        for key in data.keys():
            if key == 'Candidates':
                for c in data.get(key):
                    ...
                    candidate_mentions[c]['mention_ngrams'].append(data['MaxNGrams'])
...
def do_prepare_mentions(candidate_mentions, config, context):
    ...
    ctx['mention_ngrams'].append(config[context]['MaxNGrams'])
        ...
        for instances in train_cands:
            for candidate in instances:
                for key, value in enumerate(candidate):
                    # p print(f'{key}:{value.context.sentence.document} \n')
                    # p print(f'{key}:{value.context.sentence.text} \n')
                    # p print(f'{key}:{value.context.sentence.page} \n')
                    # p print(f'{key}:{value.context.sentence.words} \n')
                    # p print(f'{key}:{dir(value.context)} \n')
                    docs_and_values.append({
                        "documentName": value.context.sentence.document.name,
                        # "Document": value.context.sentence.document,
