...
class MentionNgramsUid(MentionNgrams):
    def __init__(
        self, uid_parts=None, n_max=3, expand=True, split_tokens=["-", "/"]
    ):
        """MentionNgrams specifically for Aaadhaar Numbers .

        :param uid_parts: a dictionary d where d[document_name.upper()] =
            [partA, partB, ...]
        """
        super(MentionNgrams, self).__init__(n_max=n_max, split_tokens=split_tokens)
        self.uid_parts = uid_parts
        self.expander = expand_part_range if expand else (lambda x: [x])
    ...
    def apply(self, doc):
        for ts in MentionNgrams.apply(self, doc):
            ...
            parts = set(enumerated_parts)
            if self.uid_parts:
                possible_parts = self.uid_parts[ts.parent.document.name.upper()]
                for base_part in enumerated_parts:
                    for part in possible_parts:
                        if part.startswith(base_part) and len(base_part) >= 4:
                            parts.add(part)
...
class MentionNgramsDoB(MentionNgrams):
...
class MentionNgramsEmail(MentionNgrams):
    def __init__(self, n_max=1, split_tokens=["-", "/"]):
        super(MentionNgrams, self).__init__(n_max=n_max, split_tokens=split_tokens)
...
class MentionNgramsPassport(MentionNgrams):
    def __init__(self, n_max=1, split_tokens=["-", "/"]):
        super(MentionNgrams, self).__init__(n_max=n_max, split_tokens=split_tokens)
        ...
        for ts in MentionNgrams.apply(self, doc):
            if ts.get_span().endswith(".0"):
                ...
                yield TemporaryImplicitSpanMention(
                    ...
                    char_start=ts.char_start,
                    char_end=ts.char_end,
                    expander_key="passport_expander",
                    position=0,
                    text=value,
                    words=[value],
                    lemmas=[value],
...
class MentionNgramsPAN(MentionNgrams):
    def __init__(self, n_max=1, split_tokens=["-", "/"]):
        super(MentionNgrams, self).__init__(n_max=n_max, split_tokens=split_tokens)
