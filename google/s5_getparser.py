...
def train_and_test(document_context=None, context_label='', user=0, pdf_path=''):
    ...
    candidate_mentions = do_prepare_mentions({}, config, context_label)
    '''
    Find all instances of 'context_label' e.g. email_id in the extracted data from all documents
    '''
    # candidate_mentions = do_prepare_mentions_batch({}, config)
    candidates = do_process_get_candidates(candidate_mentions)
    '''
    Make sure that the candidates are indeed what they seem like. e.g. are the identified values 
    indeed email ids?
    Eliminate false positives by applying the throttler functions.
    Update finalized items in the DB
    '''
    results = []
    if candidates is not None:
        results = do_get_docs_values(candidates, document_context, context_label)
        '''
        Prepare and format results from the DB
        '''
    return results
...
for fn in filenames:
    ...
    for ec in exc_context_list:
        result = train_and_test(document_context=fn, context_label=ec)
        if len(result) > 0:
            combined_results.append(result)
print(json.dumps({"result": combined_results, "job_id": job_id }))